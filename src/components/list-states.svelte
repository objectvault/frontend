<script lang="ts">
  ...
  import slUtilities from "../objects/state-list";
  ...
  export let list: TModelStateList;
  ...
  function getListActions(l: TModelStateList): TAction[] {
    const a: TAction[] = slUtilities.listActions(l);
    a.push({
      id: "list.reload",
  ...
  function onBindFilterValue(e: InputEvent) {
    ...
    // Cleanup Value
    value = slUtilities.filterNormalize(list, value);
  ...
  async function onStateChange(e: Event, entry: any) {
    try {
      // Cycle to Next State
      await list.changeNextState(entry);

      // Update List Entries
      entries = list.entries();
    } catch (e) {
      console.error(`Error Changing State of Template [${entry}]`);
    }
  }
...
<section class={sectionClasses}>
  ...
  {#if slUtilities.displayFilter(list)}
    <div class="card-header">
      <InputGroup>
        <InputGroupText>
          {slUtilities.filterTitle(list, "Filter")}
        </InputGroupText>
        <Input
          type="text"
          placeholder={slUtilities.filterPlaceHolder(list, "Filter")}
          aria-label={slUtilities.filterPlaceHolder(list, "Filter")}
  ...
  <ul class="list-group list-group-flush">
    ...
    {#each entries as entry}
      <li class="list-group-item d-flex">
        <div
          name={list.entryID(entry)}
          class="col align-self-center text-warning"
        >
          <Icon name={list.entryIcon(entry)} />
          <span class="text-secondary">{list.entryLabel(entry)}</span>
        </div>
        {#if list.displayState}
          <div name="state" class="col-auto">
            <Button
              name={`state-${list.entryID(entry)}`}
              outline={true}
              class="border-0"
              on:click={(e) => onStateChange(e, entry)}
            >
              <Icon name={list.stateIcon(list.entryState(entry))} />
            </Button>
          </div>
        {/if}
      </li>
